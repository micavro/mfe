"""
MFE 全局配置：verbose 等，优先从环境变量读取，其次从 mfe_config.yaml。
"""
    ...
    for path in [
        ...
        if os.path.isfile(path):
            try:
                ...
                pass
            break
    env = os.environ.get("MFE_VERBOSE", "").lower()
...
def is_verbose() -> bool:
    """是否输出 verbose 日志。环境变量 MFE_VERBOSE 优先于 mfe_config.yaml。"""
    return _load_config().get("verbose", False)


def set_verbose(verbose: bool) -> None:
    """设置 verbose（写入环境变量，供子进程继承）。"""
